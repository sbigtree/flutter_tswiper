...
class ColorPainter extends BasePainter {
  ...
  void draw(Canvas canvas, double space, double size, double radius) {
    ...
    double secondOffset = index == widget.count - 1
        ? radius
        : radius + ((index + 1) * (size + space));
//      print('ColorPainter-anima-$page | $index | ${widget.count} |$progress' );
...
//    print('widget.layout--${widget.layout}');
    index = widget.controller.animation.value.floor();

    switch (widget.layout) {
      case PageIndicatorLayout.NONE:
        return new NonePainter(
            widget, widget.controller.animation.value ?? 0.0, index, _paint);
  ...
  Widget build(BuildContext context) {
    Widget child = new SizedBox(
      ...
      height: widget.size,
      child: AnimatedBuilder(animation: widget.controller.animation, builder: (BuildContext context,Widget child){
//        print('widget.controller.animation.value-->>${widget.controller.animation.value}');
  ...
  void _onController() {
//    double page = widget.controller.offset ?? 0.0;
    double page = widget.controller.animation.value ?? 0.0;
    index = page.floor();
    print('_onController----$page');
    setState(() {});
  }
