...
//      print('renderPage--->>$renderPage| $itemCount | $page');
...
//    _fromIndex = _activeIndex = _pageController.getRenderIndexFromRealIndex(_pageController.initialPage);
//    print('_pageController.initialPage---->${_pageController.initialPage} -- ${_activeIndex}');
    _controller = widget.controller;
    if (_controller != null) {
      _controller.animation.addListener(onChangeNotifier);
    }
    super.initState();
  }
  ...
  void didUpdateWidget(TransformerPageView oldWidget) {
    ...
    if (_controller != getNotifier()) {
      if (_controller != null) {
        _controller.animation.removeListener(onChangeNotifier);
      }
...
//    print('currentIndex---->>$currentIndex');
  ...
  // 动画是否完成
  bool _isAnimateComplete = true;

  void onChangeNotifier() {
    int event = widget.controller.event;
    int index;
//    print('_controller.index == _pageController.page--${_controller.index}|${_pageController.page.floor()}');
    if (!_isAnimateComplete||_controller.index == _pageController.page.floor()) {
      return null ;
    }
//    return null ;
    switch (event) {
...
//          print('NEXT---->>${index} | ${_pageController.page}');
        }
        break;
      default:
        //ignore this event
...
//    return null;
    if (widget.controller.isAnimation) {
//      _controller.animateTo(index%widget.itemCount);
      _isAnimateComplete = false;
      _pageController
          .animateToPage(index,
              duration: widget.duration, curve: widget.curve ?? Curves.ease)
          .whenComplete((){
        _isAnimateComplete = true;
      });
    } else {
      _controller.animateTo(index%widget.itemCount,duration: Duration.zero);
